import '../Head-Traceability/calendarbox.css';
...
export default function CalendarBox({ selectedDate, onDateChange, selectedShift }: CalendarProps) {
  ...
  useEffect(() => {
    const fetchScheduledDates = async () => {
      try {
        const res = await fetch(`http://192.168.0.20:4001/api/calendar?month=${month + 1}&year=${year}`);
        ...
        data.forEach((item: { DATE: string }) => {
          ...
          if (m === month + 1 && y === year) daysSet.add(d);
        });
...
const handleDateClick = async (day: number) => {
  ...
  try {
    const kickRes = await fetch('http://192.168.0.20:4001/api/kickIOCounts', {
      method: 'POST',
    ...
    const kickInEl = document.getElementById('kick_io_kick_in');
    const kickOutEl = document.getElementById('kick_io_kick_out');
    ...
    const shiftRes = await fetch('http://192.168.0.20:4001/api/shiftCounts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ date: dateStr, shift: selectedShift }),
    });
    ...
    const shiftInEl = document.getElementById('kick_io_shift_in');
    const shiftOutEl = document.getElementById('kick_io_shift_out');
  ...
  return (
    <div className="cal-box">
      <div className="cal-header">
        <span className="backwardarrow" onClick={prevMonth}>
          ◄
        </span>
        <select value={month} onChange={e => setMonth(parseInt(e.target.value))} className="month-dropdown">
        ...
        <select value={year} onChange={e => setYear(parseInt(e.target.value))} className="year-dropdown">
          {yearOptions.map(y => (
        ...
        <span className="forwardarrow" onClick={nextMonth}>
          ►
        </span>
      </div>
      ...
      <div className="cal-grid">
        {grid.map((day, i) => {
          ...
          return (
            <div
              key={`day-${day}`}
              className={`cal-cell ${isActive ? 'selected-date' : 'inactive'} ${
                isSelected ? 'selected' : ''
              }`}
