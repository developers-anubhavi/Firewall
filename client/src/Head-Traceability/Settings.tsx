import '../Head-Traceability/settings.css';
...
export default function Settings({ username, logout, usertype }: SettingsProps) {
    ...
    useEffect(() => {
        fetch('http://192.168.0.20:4001/api/head_shifts')
            .then(res => res.json())
            .then(data => {
                setShiftTimes(data);
    ...
    const handleUpdate = async () => {
        ...
        try {
            const res = await fetch('http://192.168.0.20:4001/api/head_update-shifts', {
                method: 'POST',
...
const handleDownload = async () => {
  ...
  try {
    const response = await fetch("http://192.168.0.20:4001/api/fetch-data", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        engineNumber,       // <--- your search bar value
        fromDate: from,
        toDate: to,
    ...
    a.href = url;
    a.download = "HEAD_TRACEABILITY_Report.xlsx";
    a.click();
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                const res = await fetch("http://192.168.0.20:4001/api/head_table-data");
    ...
    const handleSetTable = async () => {
        ...
        for (let i = 1; i <= 9; i++) {
            const c2 = parseFloat(document.getElementById(`head_col${i}_2`)?.innerText || "0");
            const c3 = parseFloat(document.getElementById(`head_col${i}_3`)?.innerText || "0");
            const c4 = parseFloat(document.getElementById(`head_col${i}_4`)?.innerText || "0");
        ...
        try {
            const res = await fetch("http://192.168.0.20:4001/api/head_update-table", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
       ...
       <div className="settings_modal">
         ...
         {/* <div className="settings">
           ...
           <tbody>
             ...
             <tr>
                 <td id='head_col1'>MACHINE</td>
                 <td id='head_col2'>2L</td>
                 <td id='head_col3'>1.5L</td>
                 <td id='head_col4'>+/-</td></tr>
             <tr>
                 <td id='head_col1_1'>Stem Oil Seal</td>
                 <td id='head_col1_2'></td>
                 <td id='head_col1_3'></td>
                 <td id='head_col1_4'></td></tr>
             <tr>
                 <td id='head_col2_1'>Stem Oil Insp</td>
                 <td id='head_col2_2'></td>
                 <td id='head_col2_3'></td>
                 <td id='head_col2_4'></td></tr>
             <tr>
                 <td id='head_col3_1'>Cotter Retainer Assy</td>
                 <td id='head_col3_2'></td>
                 <td id='head_col3_3'></td>
                 <td id='head_col3_4'></td></tr>
             <tr>
                 <td id='head_col4_1'>Cotter Retainer Insp</td>
                 <td id='head_col4_2'></td>
                 <td id='head_col4_3'></td>
                 <td id='head_col4_4'></td></tr>
             <tr>
                 <td id='head_col5_1'>Plug Tube Press Fit</td>
                 <td id='head_col5_2'></td>
                 <td id='head_col5_3'></td>
                 <td id='head_col5_4'></td></tr>
             <tr>
                 <td id='head_col6_1'>Spark Plug Tight</td>
                 <td id='head_col6_2'></td>
                 <td id='head_col6_3'></td>
                 <td id='head_col6_4'></td></tr>
             <tr>
                 <td id='head_col7_1'>Port Leak</td>
                 <td id='head_col7_2'></td>
                 <td id='head_col7_3'></td>
                 <td id='head_col7_4'></td></tr>
             <tr>
                 <td id='head_col8_1'>Fuel Leak</td>
                 <td id='head_col8_2'></td>
                 <td id='head_col8_3'></td>
                 <td id='head_col8_4'></td></tr>
             <tr>
                 <td id='head_col9_1'>End Cap Vision</td>
                 <td id='head_col9_2'></td>
                 <td id='head_col9_3'></td>
                 <td id='head_col9_4'></td></tr>
           </tbody>
         </table>
...
<div className="excel-container">
  <h1 className="excel-title">Head Traceability - Excel Export</h1>
  ...
  <div className="headmodel-excel-section">
    <h3>MODEL</h3>
    <div className="excel-checkbox-vertical">
      <label>
        <input type="checkbox" onChange={() => handleModelChange("002")} />
        002 - 2.0L HV
      </label>
      <label>
        <input type="checkbox" onChange={() => handleModelChange("001")} />
        001 - 2.0L CV
      </label>
      <label>
        <input type="checkbox" onChange={() => handleModelChange("012")} />
        012 - 1.5L HV
      </label>
    </div>
  </div>
...
<div className="engine-search">
  ...
  <div className="excel-section">
    ...
    <div className="excel-grid">
      {[
        "Stem Oil Assembly",
        "Stem Oil Inspection",
        "Cotter/Retainer Assembly",
        "Cotter/Retainer Inspection",
        "Plug Tube Press",
        "Spark Plug Tight",
        "Port Leak Tester",
        "Fuel Leak Tester",
  ...
  >
    {loading ? "Generating..." : "Download Excel"}
        ...
        <div className='head_shifttimings'>
          <table id='shifttext'>
