import '../camhsgfirewall/settings.css';
...
export default function Settings({ username, logout, usertype }: SettingsProps) {
    ...
    useEffect(() => {
        fetch('http://192.168.0.20:4001/api/camhsg_shifts')
            .then(res => res.json())
            .then(data => {
                setShiftTimes(data);
    ...
    const handleUpdate = async () => {
        ...
        try {
            const res = await fetch('http://192.168.0.20:4001/api/camhsg_update-shifts', {
                method: 'POST',
...
const [serialNumber, setSerialNumber] = useState("");
 ...
 const handleStationChange = (value: string) => { setSelectedStations(prev => prev.includes(value) ? prev.filter(x => x !== value) : [...prev, value] );
  ...
  setLoading(true); 
  try {
     const response = await fetch("http://192.168.0.20:4001/api/fetch-cam-data", 
      { method: "POST",
         headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ serialNumber,fromDate: from, toDate: to, models: selectedModels, stations: selectedStations }) 
        }); 
        const blob = await response.blob(); 
        
        const url = URL.createObjectURL(blob); 
        const a = document.createElement("a"); 
        a.href = url; a.download = "CAM_HOUSING_Report.xlsx"; 
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                const res = await fetch("http://192.168.0.20:4001/api/camhsg_table-data");
    ...
    const handleSetTable = async () => {
        ...
        for (let i = 1; i <= 9; i++) {
            const c2 = parseFloat(document.getElementById(`camhsg_col${i}_2`)?.innerText || "0");
            const c3 = parseFloat(document.getElementById(`camhsg_col${i}_3`)?.innerText || "0");
            const c4 = parseFloat(document.getElementById(`camhsg_col${i}_4`)?.innerText || "0");
        ...
        try {
            const res = await fetch("http://192.168.0.20:4001/api/camhsg_update-table", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
       ...
       <div className="settings_modal">
         ...
         {/* <div className="settings">
           ...
           <tbody>
             ...
             <tr>
                 <td id='camhsg_col1'>MACHINE</td>
                 <td id='camhsg_col2'>2L</td>
                 <td id='camhsg_col3'>1.5L</td>
                 <td id='camhsg_col4'>+/-</td></tr>
             <tr>
                 <td id='camhsg_col1_1'>Grease Application</td>
                 <td id='camhsg_col1_2'></td>
                 <td id='camhsg_col1_3'></td>
                 <td id='camhsg_col1_4'></td></tr>
             <tr>
                 <td id='camhsg_col2_1'>Cam-cap tightening</td>
                 <td id='camhsg_col2_2'></td>
                 <td id='camhsg_col2_3'></td>
                 <td id='camhsg_col2_4'></td></tr>
...
<div className="excel-container">
  <h1 className="excel-title">Cam-Housing - Excel Export</h1>
  ...
  <div className="excel-form-group">
   ...
   <div className="cam-engine-search">
  ...
  <div className="cam_excel-section">
    <h3>MODEL</h3>
  ...
  >
    {loading ? "Generating..." : "Download Excel"}
        ...
        <div className='cam_shifttimings'>
          <table id='shifttext'>
