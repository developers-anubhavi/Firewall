import '../Piston-firewall/settings.css';
...
export default function Settings({ username, logout, usertype }: SettingsProps) {
    ...
    useEffect(() => {
        fetch('http://192.168.0.20:4001/api/piston_shifts')
            .then(res => res.json())
            .then(data => {
                setShiftTimes(data);
    ...
    const handleUpdate = async () => {
        ...
        try {
            const res = await fetch('http://192.168.0.20:4001/api/piston_update-shifts', {
                method: 'POST',
...
const [palletNo, setPalletNo] = useState("");
 ...
 const handleStationChange = (value: string) => { setSelectedStations(prev => prev.includes(value) ? prev.filter(x => x !== value) : [...prev, value] );
  ...
  setLoading(true); 
  try {
     const response = await fetch("http://192.168.0.20:4001/api/fetch-piston-data", 
      { method: "POST",
         headers: { "Content-Type": "application/json" },
          body: JSON.stringify({  palletNo,fromDate: from, toDate: to, models: selectedModels, stations: selectedStations }) 
        }); 
        const blob = await response.blob(); 
        
        const url = URL.createObjectURL(blob); 
        const a = document.createElement("a"); 
        a.href = url; a.download = "PISTON_FIREWALL_Report.xlsx"; 
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                const res = await fetch("http://192.168.0.20:4001/api/piston_table-data");
    ...
    const handleSetTable = async () => {
        ...
        for (let i = 1; i <= 9; i++) {
            const c2 = parseFloat(document.getElementById(`piston_col${i}_2`)?.innerText || "0");
            const c3 = parseFloat(document.getElementById(`piston_col${i}_3`)?.innerText || "0");
            const c4 = parseFloat(document.getElementById(`piston_col${i}_4`)?.innerText || "0");
        ...
        try {
            const res = await fetch("http://192.168.0.20:4001/api/piston_update-table", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
       ...
       <div className="settings_modal">
         ...
         {/* <div className="settings">
           ...
           <tbody>
             ...
             <tr>
                 <td id='piston_col1'>MACHINE</td>
                 <td id='piston_col2'>2L</td>
                 <td id='piston_col3'>1.5L</td>
                 <td id='piston_col4'>+/-</td></tr>
             <tr>
                 <td id='piston_col1_1'>Vision System</td>
                 <td id='piston_col1_2'></td>
                 <td id='piston_col1_3'></td>
                 <td id='piston_col1_4'></td></tr>
             <tr>
                 <td id='piston_col2_1'>Piston Rank</td>
                 <td id='piston_col2_2'></td>
                 <td id='piston_col2_3'></td>
                 <td id='piston_col2_4'></td></tr>
             <tr>
                 <td id='piston_col3_1'>Pin Assembly</td>
                 <td id='piston_col3_2'></td>
                 <td id='piston_col3_3'></td>
                 <td id='piston_col3_4'></td></tr>
             <tr>
                 <td id='piston_col4_1'>Snap Ring Inspection</td>
                 <td id='piston_col4_2'></td>
                 <td id='piston_col4_3'></td>
                 <td id='piston_col4_4'></td></tr>
             <tr>
                 <td id='piston_col5_1'>Snap Ring Vision System</td>
                 <td id='piston_col5_2'></td>
                 <td id='piston_col5_3'></td>
                 <td id='piston_col5_4'></td></tr>
             <tr>
                 <td id='piston_col6_1'>Ring Assembly</td>
                 <td id='piston_col6_2'></td>
                 <td id='piston_col6_3'></td>
                 <td id='piston_col6_4'></td></tr>
             <tr>
                 <td id='piston_col7_1'>Bearing Assembly</td>
                 <td id='piston_col7_2'></td>
                 <td id='piston_col7_3'></td>
                 <td id='piston_col7_4'></td></tr>
...
<div className="excel-container">
  <h1 className="excel-title">Piston Firewall - Excel Export</h1>
  ...
  <div className="pistonmodel_excel-section">
    <h3>MODEL</h3>
    <div className="excel-checkbox-vertical">
      ...
      <label>
        <input type="checkbox" onChange={() => handleModelChange("004")} />
        004 - 1.5L HV
      </label>
    </div>
  </div>

   <div className="piston-engine-search">
...
/>
  ...
  >
    {loading ? "Generating..." : "Download Excel"}
        ...
        <div className='piston_shifttimings'>
          <table id='shifttext'>
