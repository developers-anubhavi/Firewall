...
router.post('/head_update-table', (req, res) => {
  try {
...
router.get('/head_table-data', async (req, res) => {
  try {
...
async function getTableData() {
  try {
    ...
    for (let i = 0; i < 13; i++) {
      const row = rows[i] || {};
      tableData[`head_col${i + 1}_2`] = row.STD_TIME_2L || 0;
      tableData[`head_col${i + 1}_3`] = row.STD_TIME_1_5L || 0;
      tableData[`head_col${i + 1}_4`] = timeDevValues[i] || 0;
    }
...
function setupheadWebSocket(server){
...
module.exports = { router, setupheadWebSocket };