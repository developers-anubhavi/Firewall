...
router.post('/camhsg_update-table', (req, res) => {
  try {
...
router.get('/camhsg_table-data', async (req, res) => {
  try {
...
async function getTableData() {
  try {
    ...
    for (let i = 0; i < 13; i++) {
      const row = rows[i] || {};
      tableData[`camhsg_col${i + 1}_2`] = row.STD_TIME_2L || 0;
      tableData[`camhsg_col${i + 1}_3`] = row.STD_TIME_1_5L || 0;
      tableData[`camhsg_col${i + 1}_4`] = timeDevValues[i] || 0;
    }
...
function setupcamhsgWebSocket(server){
...
module.exports = { router, setupcamhsgWebSocket };