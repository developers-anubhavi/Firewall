...
router.get('/etshiftindata', async (req, res) => {
  try {
...
router.get('/etshiftDataOut', async (req, res) => {
  try {
    const pool = await poolPromise;
    const shiftTimes = readShiftTimes();
    const { shiftNumber, shiftStart, shiftEnd } = getCurrentShift(shiftTimes);
...
router.get('/et_kickin', async (req, res) => {
  try {
    const pool = await poolPromise;
    const shiftTimes = readShiftTimes();
    const { shiftNumber, shiftStart, shiftEnd } = getCurrentShift(shiftTimes);
    ...
    const kickInCount = result.recordset.reduce((acc, row) => acc + (row.kickInCount || 0), 0);
...
router.get('/et_kickout', async (req, res) => {
  try {
    const pool = await poolPromise;
    const shiftTimes = readShiftTimes();
    const { shiftNumber, shiftStart, shiftEnd } = getCurrentShift(shiftTimes);
    ...
    const kickOutCount = result.recordset.reduce((acc, row) => acc + (row.kickOutCount || 0), 0);
...
router.get('/etabnormalCycleData', async (req, res) => {
  try {
    const pool = await poolPromise;
    const shiftTimes = readShiftTimes();
    const { shiftNumber, shiftStart, shiftEnd } = getCurrentShift(shiftTimes);
